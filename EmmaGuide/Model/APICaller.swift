...
class APICaller {
    ...
    func fetchRestaurantDetails(placeId: String) -> Void {
        let jsonURLString = "https://maps.googleapis.com/maps/api/place/details/json?placeid=\(placeId)&key=APIKEY&language=da"
    ...
    func createRestaurant(placeId: String) -> Restaurants? {
        ...
        if self.rest != nil {
            self.rest.formatted_address = self.rest.formatted_address.components(separatedBy: ", Danmark")[0]
